...
escape_string :: proc(str: string) -> (string, bool) #no_bounds_check {
	...
	for i := 0; i < len(str); {
		char, skip := inline utf8.decode_rune(transmute([]u8)(str[i:]));
		i += skip;
	...
	for i := 0; i < len(str); {
		...
		switch char {
			...
			switch char {
				...
				c0, s0 := inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[2] = (u8)(c0); i += s0;
				c1, s1 := inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[3] = (u8)(c1); i += s1;
				c2, s2 := inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[4] = (u8)(c2); i += s2;
				c3, s3 := inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[5] = (u8)(c3); i += s3;
				ibb, _ := strconv.parse_int(string(hex[:]));
				lo = rune(ibb);

				if inline utf16.is_surrogate(lo) {
					c0, s0 := inline utf8.decode_rune(transmute([]u8)(str[i:])); i += s0;
					c1, s1 := inline utf8.decode_rune(transmute([]u8)(str[i:])); i += s1;
					...
					if c0 == '\\' && c1 == 'u' {
						...
						c2, s2 = inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[4] = (u8)(c2); i += s2;
						c3, s3 = inline utf8.decode_rune(transmute([]u8)(str[i:])); hex[5] = (u8)(c3); i += s3;                      
						obb, _ := strconv.parse_u64(string(hex[:]));
						hi = rune(obb);
...
parse :: proc(using parser: ^Parser) -> (Value, bool) {
	...
	#partial switch token.kind {
		...
		case INT:    { res, _  := strconv.parse_i64(token.text); return res, true};
		case FLOAT:  { res, _  := strconv.parse_f64(token.text); return res, true};
...
marshal :: proc(data: any, allocator := context.allocator) -> (Value, bool) {
	...
	#partial switch v in type_info.variant {
		...
		for val in v.values {
			return fmt.enum_value_to_string(val);
			/*
			switch vv in val {
	...
	switch v in value {
		...
		#partial switch variant in type_info.variant {
			...
			if len(v) > variant.count {
				fmt.eprintf("Too many elements to fit array");
				return false; // @error
			}
			...
			if array.data == nil {
				...
				fmt.eprintf("Too many elements to fit slice");
				return false; // @error
			}
		...
		#partial switch variant in type_info.variant {
			...
			for name in variant.names {
				bv := data;
				bv.id = runtime.typeid_core(variant.base.id);
				if name == string(v) {
					switch val in &bv {
						case rune:    mem.copy(data.data, &val, size_of(rune));
						case i8:      mem.copy(data.data, &val, size_of(i8));
						case i16:     mem.copy(data.data, &val, size_of(i16));
						case i32:     mem.copy(data.data, &val, size_of(i32));
						case i64:     mem.copy(data.data, &val, size_of(i64));
						case int:     mem.copy(data.data, &val, size_of(int));
						case u8:      mem.copy(data.data, &val, size_of(u8));
						case u16:     mem.copy(data.data, &val, size_of(u16));
						case u32:     mem.copy(data.data, &val, size_of(u32));
						case u64:     mem.copy(data.data, &val, size_of(u64));
						case uint:    mem.copy(data.data, &val, size_of(uint));
						case uintptr: mem.copy(data.data, &val, size_of(uintptr));
					}
